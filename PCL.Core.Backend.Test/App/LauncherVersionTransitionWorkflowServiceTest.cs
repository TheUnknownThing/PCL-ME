...
public sealed class LauncherVersionTransitionWorkflowServiceTest
{
    ...
    public void BuildPlanProducesSettingWritesLogsAndCustomSkinMigration()
    {
        ...
        Assert.AreEqual("[Start] 最高版本号从 200 升高到 400", result.HighestVersionLogMessage);
        ...
        Assert.AreEqual("[Start] 已解除帮助页面的隐藏", result.SetupAboutUnhideLogMessage);
        Assert.AreEqual("[Start] 已从老版本迁移 Mod 命名设置", result.ModNameMigrationLogMessage);
    }
