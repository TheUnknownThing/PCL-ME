...
Public Class PageInstanceOverall
    ...
    Private Sub Reload()
        ...
        Dim Logo As String = States.Instance.LogoPath(instance.PathInstance)
        Dim LogoCustom As Boolean = States.Instance.IsLogoCustom(instance.PathInstance)
        If LogoCustom Then
    ...
    Private Sub GetInstanceInfo()
        ...
        loaders.Add(New LoaderTask(Of Integer, Integer)("获取可能的整合包信息", Sub()
                                                                          Dim modpackId = States.Instance.ModpackId(PageInstanceLeft.Instance.PathInstance)
                                                                          If Not String.IsNullOrWhiteSpace(modpackId) Then
                                                                              Dim compProjects = CompRequest.GetCompProjectsByIds(New List(Of String) From {States.Instance.ModpackId(PageInstanceLeft.Instance.PathInstance)})
        ...
        loaders.Add(New LoaderTask(Of Integer, Integer)("获取实例信息", Sub()
                                                                      RunInUi(Sub()
                                                                                  ...
                                                                                  Dim launchCount = States.Instance.LaunchCount(instance.PathInstance)
                                                                                  ...
                                                                                  Else
                                                                                      items.Add(New MyListItem With {.Title = "启动次数", .Info = "已启动 " & States.Instance.LaunchCount(instance.PathInstance).ToString() & " 次", .Logo = "pack://application:,,,/images/Blocks/RedstoneLampOn.png"})
                                                                                  End If
                                                                                  If Not String.IsNullOrWhiteSpace(States.Instance.ModpackVersion(instance.PathInstance)) Then items.Add(New MyListItem With {.Title = "整合包版本", .Info = States.Instance.ModpackVersion(instance.PathInstance), .Logo = "pack://application:,,,/images/Blocks/CommandBlock.png"})
...
#Region "卡片：个性化"
    ...
    Private Sub ComboDisplayType_SelectionChanged(sender As Object, e As SelectionChangedEventArgs) Handles ComboDisplayType.SelectionChanged
        ...
        If ComboDisplayType.SelectedIndex <> 1 Then
            ...
            Try
                ...
                PageInstanceLeft.Instance.DisplayType = States.Instance.CardType(PageInstanceLeft.Instance.PathInstance)
        ...
        Else
            ...
            Try
                ...
                End If
                States.Instance.CardType(PageInstanceLeft.Instance.PathInstance) = CInt(McInstanceCardType.Hidden)
                WriteIni(McFolderSelected & "PCL.ini", "InstanceCache", "") '要求刷新缓存
                LoaderFolderRun(McInstanceListLoader, McFolderSelected, LoaderFolderRunType.ForceRun, MaxDepth:=1, ExtraPath:="versions\")
            Catch ex As Exception
    ...
    Private Sub BtnDisplayDesc_Click(sender As Object, e As EventArgs) Handles BtnDisplayDesc.Click
        Try
            Dim OldInfo As String = States.Instance.CustomInfo(PageInstanceLeft.Instance.PathInstance)
            Dim NewInfo As String = MyMsgBoxInput("更改描述", "修改实例的描述文本，留空则使用 PCL 的默认描述。", OldInfo, New ObjectModel.Collection(Of Validate), "默认描述")
            If NewInfo IsNot Nothing AndAlso OldInfo <> NewInfo Then States.Instance.CustomInfo(PageInstanceLeft.Instance.PathInstance) = NewInfo
    ...
    Private Sub ComboDisplayLogo_SelectionChanged(sender As Object, e As SelectionChangedEventArgs) Handles ComboDisplayLogo.SelectionChanged
        ...
        Try
            ...
            States.Instance.IsLogoCustom(PageInstanceLeft.Instance.PathInstance) = Not NewLogo = ""
            '刷新显示
            WriteIni(McFolderSelected & "PCL.ini", "InstanceCache", "") '要求刷新缓存
            PageInstanceLeft.Instance = New McInstance(PageInstanceLeft.Instance.Name).Load()
            Reload()
    ...
    Private Sub BtnDisplayStar_Click(sender As Object, e As EventArgs) Handles BtnDisplayStar.Click
        Try
            States.Instance.Starred(PageInstanceLeft.Instance.PathInstance) = Not PageInstanceLeft.Instance.IsStar
            PageInstanceLeft.Instance = New McInstance(PageInstanceLeft.Instance.Name).Load()
            Reload()
