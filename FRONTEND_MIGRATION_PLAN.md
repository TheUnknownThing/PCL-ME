...
- `FormMain.xaml.vb` no longer owns version-transition migration policy, version-isolation migration policy, startup open-count milestone policy, startup consent-thread orchestration, loader-init thread orchestration, or startup update-log prompt policy; milestone / version-transition shell application now route through `ModMainWindowStartupShell.vb` and startup background-thread orchestration now routes through `ModMainWindowStartupThreadShell.vb`, while it still owns WPF startup presentation and shell adapters
