...
internal sealed partial class LauncherViewModel
{
    ...
    {
        if (!IsCurrentStandardRightPane(StandardRightPaneKind.InstanceExport))
        {
            return;
        }
    ...
    private async Task SaveInstanceExportConfigAsync()
    {
        ...
        try
        {
            outputPath = await _launcherActionService.PickSaveFileAsync(
        ...
        try
        {
            ...
            _launcherActionService.PersistSharedValue(InstanceExportConfigCacheKey, outputPath);
            if (_launcherActionService.TryRevealExternalTarget(outputPath, out var error))
            {
