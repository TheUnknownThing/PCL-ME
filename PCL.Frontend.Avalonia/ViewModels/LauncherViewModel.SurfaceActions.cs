...
internal sealed partial class LauncherViewModel
{
    ...
    {
        if (IsCurrentStandardRightPane(StandardRightPaneKind.DownloadInstall) && string.Equals(command, "refresh", StringComparison.Ordinal))
        {
            ResetDownloadInstallSurface();
            return;
        }

        if (IsCurrentStandardRightPane(StandardRightPaneKind.DownloadResource) && string.Equals(command, "refresh", StringComparison.Ordinal))
        {
            ResetDownloadResourceFilters();
            return;
        }

        if (IsCurrentStandardRightPane(StandardRightPaneKind.SetupLaunch) && string.Equals(command, "reset", StringComparison.Ordinal))
        {
            ResetLaunchSettingsSurface();
            return;
        }

        if (IsCurrentStandardRightPane(StandardRightPaneKind.SetupUpdate) && string.Equals(command, "refresh", StringComparison.Ordinal))
        {
            _ = CheckForLauncherUpdatesAsync(forceRefresh: true, isUserInitiated: true);
            return;
        }

        if (IsCurrentStandardRightPane(StandardRightPaneKind.SetupFeedback) && string.Equals(command, "refresh", StringComparison.Ordinal))
        {
            _ = RefreshFeedbackSectionsAsync(forceRefresh: true);
            return;
        }

        if (IsCurrentStandardRightPane(StandardRightPaneKind.SetupGameManage) && string.Equals(command, "reset", StringComparison.Ordinal))
        {
            ResetGameManageSurface();
            return;
        }

        if (IsCurrentStandardRightPane(StandardRightPaneKind.SetupLauncherMisc) && string.Equals(command, "reset", StringComparison.Ordinal))
        {
            ResetLauncherMiscSurface();
            return;
        }

        if (IsCurrentStandardRightPane(StandardRightPaneKind.SetupJava) && string.Equals(command, "refresh", StringComparison.Ordinal))
        {
            RefreshJavaSurface();
            return;
        }

        if (IsCurrentStandardRightPane(StandardRightPaneKind.SetupUi) && string.Equals(command, "reset", StringComparison.Ordinal))
        {
            ResetUiSurface();
            return;
        }

        if (IsCurrentStandardRightPane(StandardRightPaneKind.ToolsTest) && string.Equals(command, "refresh", StringComparison.Ordinal))
        {
    ...
    {
        _launcherActionService.RemoveSharedValues(GameManageResetKeys);
    ...
    {
        _launcherActionService.RemoveLocalValues(LauncherMiscLocalResetKeys);
        _launcherActionService.RemoveSharedValues(LauncherMiscSharedResetKeys);
        _launcherActionService.RemoveSharedValues(LauncherMiscProtectedResetKeys);
    ...
    {
        _launcherActionService.RemoveLocalValues(UiLocalResetKeys);
        _launcherActionService.RemoveSharedValues(UiSharedResetKeys);
        ReloadSetupComposition();
        AddActivity(
