...
internal sealed partial class LauncherViewModel
{
    ...
    private async Task DownloadJavaRuntimeFromPromptAsync()
    {
        ...
        try
        {
            ...
            {
                var executablePath = _launcherActionService.GetJavaExecutablePath(installResult.RuntimeDirectory);
    ...
    private async Task StartLaunchAsync(bool resumeAfterPrompt = false)
    {
        try
        {
            ...
            var startResult = await Task.Run(() => _launcherActionService.StartLaunchSession(
                _launchComposition,
    ...
    private async Task EnsureLaunchRequiredArtifactsAsync(CancellationToken cancellationToken)
    {
        ...
        var task = new FrontendManagedLaunchArtifactDownloadTask(
            ...
            (progressReporter, taskCancellationToken) => Task.Run(
                () => _launcherActionService.EnsureRequiredLaunchArtifacts(
                    missingRequirements,
                    snapshot =>
                    {
    ...
    private static double ResolveLaunchArtifactProgress(LauncherActionService.FrontendLaunchArtifactProgressSnapshot snapshot)
    {
    ...
    private async Task MonitorLaunchSessionAsync(FrontendLaunchStartResult startResult)
    {
        try
        {
            ...
            await Dispatcher.UIThread.InvokeAsync(() => _launcherActionService.ApplyWatcherStopShellPlan(_launchComposition));
