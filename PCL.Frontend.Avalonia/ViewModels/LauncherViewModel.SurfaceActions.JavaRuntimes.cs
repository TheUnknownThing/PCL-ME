...
internal sealed partial class LauncherViewModel
{
    ...
    private async Task AddJavaRuntimeAsync()
    {
        ...
        try
        {
            selectedPath = await _launcherActionService.PickOpenFileAsync(
    ...
    private void SelectJavaRuntime(string key)
    {
        ...
        _launcherActionService.PersistSharedValue("LaunchArgumentJavaSelect", key == "auto" ? string.Empty : key);
        SyncJavaSelection();
    ...
    private List<JavaStorageItem> LoadStoredJavaItems()
    {
        try
        {
            var provider = _launcherActionService.RuntimePaths.OpenLocalConfigProvider();
            ...
            LogWrapper.Trace(
                SetupJavaLogModule,
                $"Loaded Java storage items from '{_launcherActionService.RuntimePaths.LocalConfigPath}': rawLength={rawJson.Length}, parsedCount={parsed.Count}.");
            return parsed;
        }
        catch (Exception ex)
        {
    ...
    private void SaveStoredJavaItems(IReadOnlyList<JavaStorageItem> items)
    {
        ...
        LogWrapper.Info(
            SetupJavaLogModule,
            $"Saving Java storage items to '{_launcherActionService.RuntimePaths.LocalConfigPath}': count={items.Count}, rawLength={rawJson.Length}.");
        _launcherActionService.PersistLocalValue("LaunchArgumentJavaUser", rawJson);
    }
}
