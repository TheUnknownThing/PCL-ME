...
internal sealed partial class LauncherViewModel
{
    ...
    {
        if (!IsCurrentStandardRightPane(StandardRightPaneKind.DownloadResource) || _currentRoute.Subpage != LauncherFrontendSubpageKey.DownloadPack)
        {
        ...
        try
        {
            ...
            var summaryDirectory = Path.Combine(_launcherActionService.RuntimePaths.FrontendArtifactDirectory, "download-installs");
    ...
    private string ResolveDownloadLauncherFolder()
    {
        ...
        return FrontendLauncherPathService.ResolveLauncherFolder(rawValue, _launcherActionService.RuntimePaths);
    }
