...
internal sealed partial class LauncherViewModel
{
    ...
    public bool ShowHelpDetailSurface => IsStandardRoute && _currentRoute.Page == LauncherFrontendPageKey.HelpDetail;
    ...
    private void ShowHelpDetail(FrontendToolsHelpEntry entry, bool addActivity)
    {
        ...
        {
            ChangeRoute(route, navigationReason, NavigationTransitionDirection.Forward);
        }
        else
        {
    ...
    private async Task CopyHelpTextAsync(string title, string? eventData)
    {
        ...
        try
        {
            await _launcherActionService.SetClipboardTextAsync(eventData);
