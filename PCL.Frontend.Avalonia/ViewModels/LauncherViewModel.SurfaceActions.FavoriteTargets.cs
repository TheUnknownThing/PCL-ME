...
internal sealed partial class LauncherViewModel
{
    private async Task ManageDownloadFavoriteTargetsAsync()
    {
        ...
        try
        {
            actionId = await _launcherActionService.PromptForChoiceAsync(
    ...
    {
        _launcherActionService.PersistSharedValue("CompFavorites", root.ToJsonString(new JsonSerializerOptions
        {
            WriteIndented = false
        }));
    ...
    private async Task ShareDownloadFavoriteTargetAsync(JsonObject target)
    {
        ...
        try
        {
            await _launcherActionService.SetClipboardTextAsync(JsonSerializer.Serialize(favoriteIds));
    ...
    private async Task ImportDownloadFavoriteTargetAsync(JsonArray root, JsonObject currentTarget)
    {
        ...
        try
        {
            shareCode = await _launcherActionService.PromptForTextAsync(
        ...
        try
        {
            destinationId = await _launcherActionService.PromptForChoiceAsync(
                T("download.favorites.targets.import.title"),
        ...
        if (string.Equals(destinationId, "new", StringComparison.Ordinal))
        {
            ...
            try
            {
                newTargetName = await _launcherActionService.PromptForTextAsync(
    ...
    private async Task RenameDownloadFavoriteTargetAsync(JsonArray root, JsonObject target, int selectedIndex)
    {
        ...
        try
        {
            nextName = await _launcherActionService.PromptForTextAsync(
    ...
    private async Task DeleteDownloadFavoriteTargetAsync(JsonArray root, JsonObject target, int selectedIndex)
    {
        ...
        try
        {
            confirmed = await _launcherActionService.ConfirmAsync(T("download.favorites.targets.delete.confirmation_title"), content, T("download.favorites.targets.delete.confirm"), isDanger: true);
        }
        catch (Exception ex)
        {
