...
internal sealed partial class LauncherViewModel
{
        ...
        bool initializeAllSurfaces = true)
    {
        ...
        ApplyInstanceComposition(
            FrontendInstanceCompositionService.Compose(_launcherActionService.RuntimePaths, loadMode, _i18n),
            initializeAllSurfaces);
    ...
    private void PersistInstanceSetting(string? propertyName)
    {
        ...
        switch (propertyName)
        {
            ...
            case nameof(SelectedInstanceOverviewCategoryIndex):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "DisplayType", SelectedInstanceOverviewCategoryIndex);
                break;
            case nameof(SelectedInstanceIsolationIndex):
                switch (SelectedInstanceIsolationIndex)
                {
                    case 0:
                        _launcherActionService.RemoveInstanceValues(instanceDirectory, ["VersionArgumentIndieV2"]);
                        break;
                    case 1:
                        _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionArgumentIndieV2", true);
                        break;
                    default:
                        _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionArgumentIndieV2", false);
                        break;
                }
                break;
            case nameof(InstanceWindowTitleSetting):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionArgumentTitle", InstanceWindowTitleSetting);
                break;
            case nameof(UseDefaultInstanceWindowTitle):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionArgumentTitleEmpty", UseDefaultInstanceWindowTitle);
                break;
            case nameof(InstanceCustomInfoSetting):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionArgumentInfo", InstanceCustomInfoSetting);
                break;
            case nameof(SelectedInstanceJavaIndex):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionArgumentJavaSelect", BuildStoredInstanceJavaSelection());
                break;
            ...
            case nameof(UseCustomInstanceRamAllocation):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionRamType", _instanceMemoryModeIndex);
                break;
            case nameof(InstanceCustomRamAllocation):
                _launcherActionService.PersistInstanceValue(
                    instanceDirectory,
                    "VersionRamCustom",
            ...
            case nameof(SelectedInstanceServerLoginRequireIndex):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionServerLoginRequire", SelectedInstanceServerLoginRequireIndex);
                break;
            case nameof(InstanceServerAuthServer):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionServerAuthServer", InstanceServerAuthServer);
                break;
            case nameof(InstanceServerAutoJoin):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionServerEnter", InstanceServerAutoJoin);
                break;
            case nameof(SelectedInstanceRendererIndex):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceRenderer", SelectedInstanceRendererIndex);
                break;
            case nameof(InstanceLaunchWrapperCommand):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceWrapper", InstanceLaunchWrapperCommand);
                break;
            case nameof(InstanceLaunchJvmArguments):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceJvm", InstanceLaunchJvmArguments);
                break;
            case nameof(InstanceLaunchGameArguments):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceGame", InstanceLaunchGameArguments);
                break;
            case nameof(InstanceClasspathHead):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceClasspathHead", InstanceClasspathHead);
                break;
            case nameof(InstanceLaunchBeforeCommand):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceRun", InstanceLaunchBeforeCommand);
                break;
            case nameof(InstanceEnvironmentVariables):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceEnvironmentVariables", InstanceEnvironmentVariables);
                break;
            case nameof(WaitForInstanceLaunchBeforeCommand):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceRunWait", WaitForInstanceLaunchBeforeCommand);
                break;
            case nameof(SelectedInstanceForceX11OnWaylandIndex):
                if (SelectedInstanceForceX11OnWaylandIndex == 0)
                {
                    _launcherActionService.RemoveInstanceValues(instanceDirectory, ["VersionAdvanceForceX11OnWayland"]);
                }
                else
                {
                    _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceForceX11OnWayland", SelectedInstanceForceX11OnWaylandIndex);
                }
                break;
            case nameof(IgnoreInstanceJavaCompatibilityWarning):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceJava", IgnoreInstanceJavaCompatibilityWarning);
                break;
            case nameof(DisableInstanceFileValidation):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceAssetsV2", DisableInstanceFileValidation);
                break;
            case nameof(FollowInstanceLauncherProxy):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceUseProxyV2", FollowInstanceLauncherProxy);
                break;
            case nameof(DisableInstanceJavaLaunchWrapper):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceDisableJLW", DisableInstanceJavaLaunchWrapper);
                break;
            case nameof(DisableInstanceRetroWrapper):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionAdvanceDisableRW", DisableInstanceRetroWrapper);
                break;
            case nameof(UseInstanceDebugLog4jConfig):
                _launcherActionService.PersistInstanceValue(instanceDirectory, "VersionUseDebugLog4j2Config", UseInstanceDebugLog4jConfig);
                break;
        }
    }
    ...
    private void OpenInstanceTarget(string activity, string? target, string emptyState)
    {
        ...
        var openSucceeded = ShouldRevealInstanceTargetInShell(target)
            ? _launcherActionService.TryRevealExternalTarget(target, out error)
            : _launcherActionService.TryOpenExternalTarget(target, out error);
        if (openSucceeded)
        {
    ...
    private void PersistInstanceOverviewIconSelection(string instanceDirectory, int selectedIndex)
    {
        ...
        {
            _launcherActionService.RemoveInstanceValues(instanceDirectory, ["Logo", "LogoCustom"]);
            return;
        }

        _launcherActionService.PersistInstanceValue(instanceDirectory, "LogoCustom", false);
        _launcherActionService.PersistInstanceValue(
            instanceDirectory,
            "Logo",
    ...
    private void RemoveDeprecatedInstanceSpecifiedServerConfigKeys()
    {
        ...
        _launcherActionService.RemoveInstanceValues(instanceDirectory, DeprecatedInstanceSpecifiedServerConfigKeys);
    }
