...
internal sealed partial class LauncherViewModel
{
    ...
    private async Task ShowAvailableUpdateDetailAsync()
    {
        ...
        if (!string.IsNullOrWhiteSpace(_updateStatus.AvailableUpdateReleaseUrl)
            && _launcherActionService.TryOpenExternalTarget(_updateStatus.AvailableUpdateReleaseUrl, out openError))
        {
    ...
    {
        _launcherActionService.RemoveLocalValues(LaunchLocalResetKeys);
        _launcherActionService.RemoveSharedValues(LaunchSharedResetKeys);
    ...
    private async Task<FrontendPreparedUpdateInstall> PrepareAvailableUpdateAsync(string target)
    {
        var request = new FrontendUpdateInstallRequest(
            ...
            ArtifactDirectory: _launcherActionService.RuntimePaths.FrontendArtifactDirectory,
            TempDirectory: _launcherActionService.RuntimePaths.FrontendTempDirectory,
            ExecutableDirectory: _launcherActionService.RuntimePaths.ExecutableDirectory,
            ProcessPath: Environment.ProcessPath,
            ProcessId: Environment.ProcessId,
            PlatformAdapter: _launcherActionService.PlatformAdapter);
    ...
    {
        if (!_launcherActionService.TryStartDetachedScript(preparedInstall.InstallerScriptPath, out var error))
        {
            AddFailureActivity(
