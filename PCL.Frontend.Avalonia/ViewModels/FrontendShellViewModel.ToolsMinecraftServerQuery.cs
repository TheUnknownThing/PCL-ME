...
internal sealed class MinecraftServerQuerySrvResource
{
    ...
    {
        var buffer = bytes.Slice(offset, length).Span;
        offset += length;
        Priority = BinaryPrimitives.ReadUInt16BigEndian(buffer[..2]);
        Weight = BinaryPrimitives.ReadUInt16BigEndian(buffer[2..4]);
        Port = BinaryPrimitives.ReadUInt16BigEndian(buffer[4..6]);

        var labels = new List<string>();
        var index = 6;
        while (index < length)
        {
            var labelLength = buffer[index];
            if (labelLength == 0)
            {
                break;
            }
