...
internal sealed partial class LauncherViewModel
{
    ...
    {
        var logDirectory = Path.Combine(_launcherActionService.RuntimePaths.LauncherAppDataDirectory, "Log");
        ...
        var exportDirectory = Path.Combine(_launcherActionService.RuntimePaths.FrontendArtifactDirectory, "log-exports");
    ...
    private void OpenLauncherLogDirectory()
    {
        ...
        if (_launcherActionService.TryOpenExternalTarget(logDirectory, out var error))
        {
    ...
    private void ExportSettingsSnapshot()
    {
        try
        {
            var exportDirectory = FrontendSettingsSnapshotWorkflowService.CreateSnapshot(_launcherActionService.RuntimePaths);
            AddActivity(LT("setup.launcher_misc.activities.export_settings"), exportDirectory);
            if (!_launcherActionService.TryOpenExternalTarget(exportDirectory, out var error))
            {
    ...
    private async Task ImportSettingsAsync()
    {
        ...
        try
        {
            sourceDirectory = await _launcherActionService.PickFolderAsync(
        ...
        try
        {
            FrontendSettingsSnapshotWorkflowService.RestoreSnapshot(_launcherActionService.RuntimePaths, sourceDirectory);
        }
        catch (Exception ex)
        {
            AddFailureActivity(LT("setup.launcher_misc.activities.import_settings_failed"), ex.Message);
    ...
    private void OpenSettingsSnapshotFolder()
    {
        ...
        if (_launcherActionService.TryOpenExternalTarget(settingsDirectory, out var error))
        {
    ...
    {
        return FrontendSettingsSnapshotWorkflowService.GetSnapshotRootDirectory(_launcherActionService.RuntimePaths);
    }
