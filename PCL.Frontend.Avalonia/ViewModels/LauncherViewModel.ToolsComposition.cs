...
internal sealed partial class LauncherViewModel
{
    ...
    {
        ApplyToolsComposition(FrontendToolsCompositionService.Compose(_launcherActionService.RuntimePaths, _i18n));
    }
    ...
    {
        ApplyHelpState(FrontendToolsCompositionService.LoadHelpState(_launcherActionService.RuntimePaths, _i18n.Locale));
    }
    ...
    private void PersistToolsSetting(string? propertyName)
    {
        ...
        switch (propertyName)
        {
            case nameof(ToolDownloadFolder):
                _launcherActionService.PersistSharedValue("CacheDownloadFolder", ToolDownloadFolder);
                break;
            case nameof(ToolDownloadUserAgent):
                _launcherActionService.PersistSharedValue("ToolDownloadCustomUserAgent", ToolDownloadUserAgent);
                break;
        }
    }
    ...
    {
        var removedCount = 0;
        removedCount += DeleteDirectorySafely(_launcherActionService.RuntimePaths.FrontendArtifactDirectory);
        removedCount += DeleteDirectorySafely(_launcherActionService.RuntimePaths.FrontendTempDirectory);
        removedCount += DeleteDirectoryContentsSafely(Path.Combine(_launcherActionService.RuntimePaths.LauncherAppDataDirectory, "Log"));
    ...
    private async Task CreateLauncherShortcutAsync()
    {
        ...
        try
        {
            ...
            {
                var selectedId = await _launcherActionService.PromptForChoiceAsync(
            ...
            var shortcutPath = _launcherActionService.CreateLauncherShortcutAt(selectedTarget.Directory, LauncherShortcutDisplayName);
    ...
    private List<ToolboxShortcutTarget> BuildShortcutTargets(string displayName)
    {
        ...
        var desktopDirectory = _launcherActionService.PlatformAdapter.TryGetDesktopDirectory();
