...
internal sealed partial class LauncherViewModel
{
        ...
        bool addActivity)
    {
        try
        {
            ...
            _launcherActionService.PersistLocalValue(HomepageCacheUrlKey, uri.ToString());
    ...
    {
        return Path.Combine(_launcherActionService.RuntimePaths.FrontendTempDirectory, "homepage-cache", "Custom.xaml");
    }
    ...
    private IEnumerable<string> EnumerateHomepageHintPaths()
    {
        ...
        foreach (var path in FrontendLaunchHintService.EnumerateExternalHintPaths(_launcherActionService.RuntimePaths.DataDirectory, _i18n.Locale))
        {
            yield return path;
        }
    }
