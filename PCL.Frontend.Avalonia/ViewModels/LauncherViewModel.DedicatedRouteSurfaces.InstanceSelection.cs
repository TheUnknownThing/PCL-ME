...
internal sealed partial class LauncherViewModel
{
    ...
    private async Task AddInstanceSelectionFolderAsync()
    {
        try
        {
            var pickedFolderPath = await _launcherActionService.PickFolderAsync(LT("shell.instance_select.shortcuts.add_folder.pick_title"));
    ...
    private async Task ImportInstanceSelectionPackAsync()
    {
        try
        {
            var sourcePath = await _launcherActionService.PickOpenFileAsync(
    ...
    private async Task StartInstanceSelectionPackInstallAsync(string sourcePath)
    {
        var launcherDirectory = string.IsNullOrWhiteSpace(_instanceSelectionLauncherDirectory)
            ? ResolveLauncherFolder(
                ReadValue(_launcherActionService.RuntimePaths.OpenLocalConfigProvider(), "LaunchFolderSelect", FrontendLauncherPathService.DefaultLauncherFolderRaw),
    ...
    private void OpenInstanceSelectionFolder(InstanceSelectionFolderSnapshot folder)
    {
        ...
        if (_launcherActionService.TryRevealExternalTarget(folder.Directory, out var error))
        {
    ...
    private void OpenInstanceSelectionEntry(InstanceSelectionSnapshot entry)
    {
        ...
        _launcherActionService.PersistLocalValue("LaunchInstanceSelect", entry.Name);
        if (!_instanceComposition.Selection.HasSelection
            || !string.Equals(
    ...
    private void ToggleInstanceSelectionFavorite(InstanceSelectionSnapshot entry)
    {
        try
        {
            ...
            _launcherActionService.PersistInstanceValue(entry.Directory, "IsStar", nextIsFavorite);
    ...
    private InstanceSelectEntryViewModel CreateInstanceSelectionEntry(InstanceSelectionSnapshot entry)
    {
        ...
        return new InstanceSelectEntryViewModel(
            ...
            new ActionCommand(() =>
            {
                if (_launcherActionService.TryOpenExternalTarget(entry.Directory, out var error))
                {
