...
internal sealed partial class DownloadResourceRightPaneView : UserControl
{
    ...
    private bool _isApplyingLauncherFilterState;

    public DownloadResourceRightPaneView()
    {
        InitializeComponent();
    ...
    private void OnLauncherPropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        if (e.PropertyName is not (
            nameof(LauncherViewModel.DownloadResourceSourceOptions)
            or nameof(LauncherViewModel.DownloadResourceTagOptions)
            or nameof(LauncherViewModel.DownloadResourceSortOptions)
            or nameof(LauncherViewModel.DownloadResourceVersionOptions)
            or nameof(LauncherViewModel.DownloadResourceLoaderOptions)
            or nameof(LauncherViewModel.SelectedDownloadResourceSourceOption)
            or nameof(LauncherViewModel.SelectedDownloadResourceTagOption)
            or nameof(LauncherViewModel.SelectedDownloadResourceSortOption)
            or nameof(LauncherViewModel.SelectedDownloadResourceVersionOption)
            or nameof(LauncherViewModel.SelectedDownloadResourceLoaderOption)))
        {
            if (e.PropertyName is nameof(LauncherViewModel.ShowDownloadResourceLoadingCard)
                or nameof(LauncherViewModel.ShowDownloadResourceContent)
                or nameof(LauncherViewModel.ShowDownloadResourceHint)
                or nameof(LauncherViewModel.DownloadResourceHintText))
            {
                ScheduleVisualStateSync();
            }
        ...
        _isApplyingLauncherFilterState = true;
        ScheduleSelectionRestore();
    }
    ...
    private async Task SyncVisualStateAsync(int version)
    {
        ...
        var showLoading = _observedLauncher?.ShowDownloadResourceLoadingCard == true;
        var showContent = _observedLauncher?.ShowDownloadResourceContent == true;
        var showHint = _observedLauncher?.ShowDownloadResourceHint == true;
        var hintText = _observedLauncher?.DownloadResourceHintText ?? string.Empty;
    ...
    private void RestoreFilterSelections(int remainingAttempts)
    {
        ...
        {
            _isApplyingLauncherFilterState = false;
            return;
        }
        ...
        try
        {
            restoredAllSelections =
                ApplyComboSelection(_sourceComboBox, _observedLauncher.SelectedDownloadResourceSourceOption)
                && ApplyComboSelection(_tagComboBox, _observedLauncher.SelectedDownloadResourceTagOption)
                && ApplyComboSelection(_sortComboBox, _observedLauncher.SelectedDownloadResourceSortOption)
                && ApplyComboSelection(_versionComboBox, _observedLauncher.SelectedDownloadResourceVersionOption)
                && ApplyComboSelection(_loaderComboBox, _observedLauncher.SelectedDownloadResourceLoaderOption);
        }
        finally
        {
    ...
    {
        if (_isRestoringFilterSelections || _isApplyingLauncherFilterState || _observedLauncher is null)
        {
            return;
        }
        ...
        switch (filterKind)
        {
            case FilterKind.Source:
                _observedLauncher.SelectedDownloadResourceSourceOption = selectedOption;
                break;
            case FilterKind.Tag:
                _observedLauncher.SelectedDownloadResourceTagOption = selectedOption;
                break;
            case FilterKind.Sort:
                _observedLauncher.SelectedDownloadResourceSortOption = selectedOption;
                break;
            case FilterKind.Version:
                _observedLauncher.SelectedDownloadResourceVersionOption = selectedOption;
                break;
            case FilterKind.Loader:
                _observedLauncher.SelectedDownloadResourceLoaderOption = selectedOption;
                break;
        }
    }
